 ...
 */
const FormComponent = ( {courses, dates, statuses, registration, onClearSelection, renderToast}) => {
    ...
    useEffect(() => {
        if (registration) {
            setSelectedCourse(registration.attendingBootcamp.key);
            setSelectedDate(registration.attendingDate.key);
            setNotes(registration.previousExperience);
    ...
    const handleFormSubmit = async (event) => {
        ...
        if (!selectedDate || selectedDate === '') {
            renderToast('Error', 'Please select a date.', 'danger');
        ...
        var method = 'POST';
        var body = JSON.stringify({attendingBootcamp: {key : selectedCourse}, attendingDate: {key: selectedDate}, previousExperience: notes});
        var url = "o/c/bootcamprequests";
        ...
        if (registration) {
            ...
            method = 'PATCH';
            url = "o/c/bootcamprequests/by-external-reference-code/" + registration.externalReferenceCode;
        }
    ...
    return (
        <ClayForm onSubmit={handleFormSubmit}>
            ...
            <ClayForm.Group>
                ...
                <ClaySelect id="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)}>
                    <ClaySelect.Option key="invalid" value="" label="-- Select a Date --" />
                    {Object.entries(dates).map(([dateId, dateName]) => (
                        <ClaySelect.Option key={dateId} value={dateId} label={dateName} />
                    ))}
                </ClaySelect>
            </ClayForm.Group>
