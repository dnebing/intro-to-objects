...
function DetailComponent({ externalReferenceCode , onClearSelection, admin = false }) {
  ...
  const fetchRegistration = () => {
    api('o/c/bootcamprequests/by-external-reference-code/' + externalReferenceCode)
  ...
  return (
    <div>
      { data && ( <div>
      <h1>Registration For {data["attendingBootcamp"]["name"]}</h1>
      <div>
      ...
      <div>
        <label>Date: </label>
        <span>{data["attendingDate"]["name"]}</span>
      </div>
      <div>
        <label>Notes: </label>
        <span>{data["previousExperience"]}</span>
      </div>
      <div>
        <label>Status: </label>
