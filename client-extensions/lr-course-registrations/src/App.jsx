...
import { RegistrationStatusERC, UpcomingCoursesERC, UpcomingCourseDatesERC } from './constants.js';
...
function App({ admin = false}) {
	...
	/**
	 * fetchUpcomingDates: Fetches a map of the upcoming course dates
	 */
	const fetchUpcomingDates = async () => {
		try {
			const map = await fetchMap('o/headless-admin-list-type/v1.0/list-type-definitions/by-external-reference-code/'
				+ UpcomingCourseDatesERC);
		
			setUpcomingDates(map);
		} catch (error) {
			console.log(error);
		}
	};
	...
	<>
		<MemoryRouter initialEntries={['/']}>
		  <Routes>
			...
			<Route path="/add" element={ <FormComponent courses={upcomingCourses} dates={upcomingDates} statuses={registrationStatuses} 
